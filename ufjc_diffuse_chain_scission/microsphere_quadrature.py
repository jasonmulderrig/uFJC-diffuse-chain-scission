...
class MicrosphereQuadratureScheme: # add functionality to access any sphere quadrature scheme that quadpy offers
    ...
    def __init__(self, microsphere_quadrature_order):
    ...
    def stereographic_projection(self, coord_i, coord_j, coord_k, norm_dir, data = None):
        if len(coord_i) != self.points_num:
            sys.exit('Error: Coordinate vectors need to contain the same number of points in the microsphere quadrature scheme.')
        elif len(coord_j) != self.points_num:
            sys.exit('Error: Coordinate vectors need to contain the same number of points in the microsphere quadrature scheme.')
        elif len(coord_k) != self.points_num:
            sys.exit('Error: Coordinate vectors need to contain the same number of points in the microsphere quadrature scheme.')
        elif data is not None:
            if len(data) != self.points_num:
                sys.exit('Error: Data vector needs to contain the same number of points in the microsphere quadrature scheme.')
        ...
        for point in range(self.points_num):
            if norm_dir == '+':
                if coord_k[point] >= 0.:
                    stereo_proj_coord_i_val = coord_i[point]
                    stereo_proj_coord_j_val = coord_j[point]

                    stereo_proj_coord_i = np.append(stereo_proj_coord_i, stereo_proj_coord_i_val)
                    stereo_proj_coord_j = np.append(stereo_proj_coord_j, stereo_proj_coord_j_val)

                    if data is not None:
                        stereo_proj_data_val = data[point]
                        stereo_proj_data     = np.append(stereo_proj_data, stereo_proj_data_val)
        ...
        if data is None:
            return stereo_proj_coord_i, stereo_proj_coord_j
        else:
            return stereo_proj_coord_i, stereo_proj_coord_j, stereo_proj_data

    # def data_stereographic_projection(self, coord_i, coord_j, coord_k, norm_dir, data):
    #     if len(coord_i) != self.points_num:
    #         sys.exit('Error: Coordinate vectors need to contain the same number of points in the microsphere quadrature scheme.')
    #     elif len(coord_j) != self.points_num:
    #         sys.exit('Error: Coordinate vectors need to contain the same number of points in the microsphere quadrature scheme.')
